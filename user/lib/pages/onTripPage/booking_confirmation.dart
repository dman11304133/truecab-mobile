...
// List rentalOption = []; // moved to ride_state.dart
// int rentalChoosenOption = 0; // moved to ride_state.dart
Animation<double>? _animation;
bool addCoupon = false;
bool isLoading = false;
// List<fmlt.LatLng> fmpoly = []; // moved to ride_state.dart
...
class _BookingConfirmationState extends State<BookingConfirmation>
  ...
  // Cached Firebase drivers query to avoid recreating on every build()
  Query? _cachedDriversQuery;
  Stream<DatabaseEvent>? _cachedDriversStream;
  ...
  DateTime? toDate;
  double _isDateTimebottom = -1500;
  dynamic _dateTimeHeight = 0;
  bool nofromdate = false;
  double maxWaitTime = 120.0;

  @override
  void initState() {
    debugPrint('📍 [BC] initState() called. widget.type=${widget.type}, addressList.length=${addressList.length}');
    fmpoly.clear();
    ...
    // Removed aggressive clear() to prevent state flapping during search/acceptance
    // if (!ismulitipleride && userRequestData['accepted_at'] != null) {
    //   userRequestData.clear();
    // }
    debugPrint('📍 [BC] calling getLocs()...');
    getLocs();
    debugPrint('📍 [BC] calling _buildDriversQuery()...');
    _buildDriversQuery();
    debugPrint('📍 [BC] initState() complete.');
  ...
  void dispose() {
    ...
    _controller?.dispose();
    _controller = null;
    _cachedDriversQuery = null;
    _cachedDriversStream = null;
  ...
  // ── Running Fare Timer ──────────────────────────────────────────────────
  void _startFareTimer() {
    _fareTimer?.cancel();
    _fareTimer = Timer.periodic(const Duration(seconds: 1), (_) {
      if (!mounted) return;
      
      final bool hasAcceptStatus = userRequestData['accepted_at'] != null || 
          userRequestData['is_accept']?.toString() == '1' ||
          userRequestData['is_accept'] == 1;

      final bool isNotCompleted = userRequestData['is_completed'] == null ||
              userRequestData['is_completed'].toString() == '0' ||
              userRequestData['is_completed'] == 0 ||
              userRequestData['is_completed'].toString() == 'false' ||
              userRequestData['is_completed'] == false;

      final bool active = userRequestData.isNotEmpty &&
          hasAcceptStatus && 
          isNotCompleted;
...
//running timer
  timer() {
    if (userRequestData['is_bid_ride'] == 1) {
      ...
      timing = userDetails['maximum_time_for_find_drivers_for_regular_ride'] != null
          ? int.tryParse(userDetails['maximum_time_for_find_drivers_for_regular_ride'].toString())
          : 120; // fallback just in case
      maxWaitTime = timing != null ? double.parse(timing.toString()) : 120.0;
      if (mounted) {
          ...
          if (timing != null) {
            if (userRequestData.isNotEmpty &&
              ...
              valueNotifierBook.incrementNotifier();
                ...
                userRequestData['accepted_at'] == null &&
                timing == 0) {
              var val = await cancelRequest(autoCancel: true);
...
//create icon
  ...
  _capturePng(GlobalKey? iconKeys) async {
    if (iconKeys == null) return null;
    dynamic bitmap;

    try {
      // Loop a few times to allow the widget to mount and paint
      int retries = 10;
      while (iconKeys.currentContext == null && retries > 0) {
        await Future.delayed(const Duration(milliseconds: 100));
        retries--;
      }

      if (iconKeys.currentContext == null) {
        debugPrint('❌ [BC] _capturePng failed: currentContext is still null after waiting');
        return null;
      }

      var renderObject = iconKeys.currentContext!.findRenderObject();
      if (renderObject is RenderRepaintBoundary) {
        ...
        debugPrint('❌ [BC] _capturePng failed: renderObject is not a RenderRepaintBoundary, it is ${renderObject.runtimeType}');
        return null;
      }
    } catch (e) {
      debugPrint('❌ [BC] _capturePng exception: $e');
    }
    return bitmap;
  }

  addDropMarker() async {
    debugPrint('📍 [BC] addDropMarker() called. addressList.length=${addressList.length}');
    if (addressList.isEmpty) return;
    for (var i = 1; i < addressList.length; i++) {
      if (iconDropKeys[i] == null) continue;
      var testIcon = await _capturePng(iconDropKeys[i]);
      if (testIcon != null && i < addressList.length) {
        setState(() {
          myMarker.add(Marker(
    ...
    if (widget.type != 1) {
      LatLngBounds bound;
      if (userRequestData.isNotEmpty) {
        debugPrint('🗺️ [USER] Calculating Map Bounds. Coordinates: pick_lat=${userRequestData['pick_lat']}, drop_lat=${userRequestData['drop_lat']}');
        try {
          if ((double.tryParse(userRequestData['pick_lat'].toString()) ?? 0.0) >
                (double.tryParse(userRequestData['drop_lat'].toString()) ?? 0.0) &&
            (double.tryParse(userRequestData['pick_lng'].toString()) ?? 0.0) >
                (double.tryParse(userRequestData['drop_lng'].toString()) ?? 0.0)) {
          bound = LatLngBounds(
              southwest: LatLng(
                  double.tryParse(userRequestData['drop_lat'].toString()) ?? 0.0,
                  double.tryParse(userRequestData['drop_lng'].toString()) ?? 0.0),
              northeast: LatLng(
                  double.tryParse(userRequestData['pick_lat'].toString()) ?? 0.0,
                  double.tryParse(userRequestData['pick_lng'].toString()) ?? 0.0));
        } else if ((double.tryParse(userRequestData['pick_lng'].toString()) ?? 0.0) >
            (double.tryParse(userRequestData['drop_lng'].toString()) ?? 0.0)) {
          bound = LatLngBounds(
              southwest: LatLng(
                  double.tryParse(userRequestData['pick_lat'].toString()) ?? 0.0,
              ...
              northeast: LatLng(
                  double.tryParse(userRequestData['drop_lat'].toString()) ?? 0.0,
                  double.tryParse(userRequestData['pick_lng'].toString()) ?? 0.0));
        } else if ((double.tryParse(userRequestData['pick_lat'].toString()) ?? 0.0) >
            (double.tryParse(userRequestData['drop_lat'].toString()) ?? 0.0)) {
          bound = LatLngBounds(
              southwest: LatLng(
                  double.tryParse(userRequestData['drop_lat'].toString()) ?? 0.0,
        ...
        debugPrint('❌ [USER] Error calculating bounds: $e');
        bound = LatLngBounds(
            southwest: LatLng(double.tryParse(userRequestData['pick_lat'].toString()) ?? 0.0,
                double.tryParse(userRequestData['pick_lng'].toString()) ?? 0.0),
            northeast: LatLng(double.tryParse(userRequestData['pick_lat'].toString()) ?? 0.0,
                double.tryParse(userRequestData['pick_lng'].toString()) ?? 0.0));
      }
    } else {
        bool hasPickup = addressList.any((element) => element.type == 'pickup');
        bool hasDrop = addressList.any((element) => element.type == 'drop');

        if (hasPickup && hasDrop) {
          var pickup = addressList.firstWhere((element) => element.type == 'pickup').latlng;
          var drop = addressList.lastWhere((element) => element.type == 'drop').latlng;

          if (pickup.latitude > drop.latitude && pickup.longitude > drop.longitude) {
            bound = LatLngBounds(southwest: drop, northeast: pickup);
            ...
            bound = LatLngBounds(southwest: pickup, northeast: drop);
          }
        } else {
          // Fallback if pickup or drop is missing
          bound = LatLngBounds(
            southwest: center,
            northeast: center,
          );
  ...
  addMarker() async {
    debugPrint('📍 [BC] addMarker() called. addressList.length=${addressList.length}, userRequestData.isEmpty=${userRequestData.isEmpty}');
    if (!mounted) return;
    var testIcon = await _capturePng(iconKey);
    if (testIcon != null && mounted) {
      LatLng? markerPosition;
      if (userRequestData.isNotEmpty) {
        markerPosition = LatLng(
            double.tryParse(userRequestData['pick_lat'].toString()) ?? 0.0,
            double.tryParse(userRequestData['pick_lng'].toString()) ?? 0.0);
      } else if (addressList.any((element) => element.type == 'pickup')) {
        markerPosition = addressList.firstWhere((element) => element.type == 'pickup').latlng;
      }
...
//add drop marker
  addPickDropMarker() async {
    debugPrint('📍 [BC] addPickDropMarker() called. mapType=$mapType, widget.type=${widget.type}, addressList.length=${addressList.length}, userRequestData.isEmpty=${userRequestData.isEmpty}');
    try {
    if (mapType == 'google') {
      addMarker();
      ...
      if (userRequestData.isNotEmpty &&
        ...
        if (userRequestData.isEmpty) {
          var pickupElement = addressList.cast<AddressList?>().firstWhere((element) => element?.id == 'pickup' || element?.type == 'pickup', orElse: () => null);
        ...
        addDropMarker();
        if (userRequestData.isEmpty) {
          var pickupElement = addressList.cast<AddressList?>().firstWhere((element) => element?.type == 'pickup', orElse: () => null);
          if (pickupElement != null) {
            polyline.add(
              Polyline(
                  polylineId: const PolylineId('1'),
      ...
      if (addressList.length > 1 &&
        ...
        _fmController.move(
            fmlt.LatLng(center.latitude, center.longitude), 13);
      }
    }
    } catch (e, stackTrace) {
      debugPrint('❌ [BC] addPickDropMarker() CRASHED: $e');
      debugPrint('❌ [BC] Stack trace: $stackTrace');
    }
  }
...
//get location permission and location details
  getLocs() async {
    debugPrint('📍 [BC] getLocs() called. addressList.length=${addressList.length}');
    ...
    setState(() {
      ...
      try {
        if (userRequestData.isNotEmpty) {
          ...
          var pickupElement = addressList.cast<AddressList?>().firstWhere((element) => element?.type == 'pickup', orElse: () => null);
          if (pickupElement != null) {
            center = pickupElement.latlng;
          } else if (addressList.isNotEmpty) {
            center = addressList.first.latlng;
          } else {
            // center remains as current location or default
          }
        }
      } catch (e) {
        debugPrint('🗺️ [MAP] ❌ center calculation error: $e');
    ...
    try {
      ...
      debugPrint('🗺️ [MAP] ❌ LocationService error: $e');
      serviceEnabled = false;
    }
...
// polyGot = true;
    ...
    // Rebuild the cached Firebase drivers query with updated location
    _buildDriversQuery();
  }
  ...
  @override
  // Build and cache the Firebase drivers query so it's not recreated on every build()
  void _buildDriversQuery() {
    GeoHasher geo = GeoHasher();

    double lat = 0.0144927536231884;
    double lon = 0.0181818181818182;

    var pickupElement = (userRequestData.isEmpty && addressList.isNotEmpty)
        ? addressList.cast<AddressList?>().firstWhere((element) => element?.type == 'pickup', orElse: () => null)
        : null;

    double lowerLat = (pickupElement != null)
        ? pickupElement.latlng.latitude - (lat * 1.24)
        : (userRequestData.isNotEmpty)
            ? (double.tryParse(userRequestData['pick_lat'].toString()) ?? 0.0) - (lat * 1.24)
            : 0.0;
    double lowerLon = (pickupElement != null)
        ? pickupElement.latlng.longitude - (lon * 1.24)
        : (userRequestData.isNotEmpty)
            ? (double.tryParse(userRequestData['pick_lng'].toString()) ?? 0.0) - (lon * 1.24)
            : 0.0;

    double greaterLat = (pickupElement != null)
        ? pickupElement.latlng.latitude + (lat * 1.24)
        : (userRequestData.isNotEmpty)
            ? (double.tryParse(userRequestData['pick_lat'].toString()) ?? 0.0) + (lat * 1.24)
            : 0.0;
    double greaterLon = (pickupElement != null)
        ? pickupElement.latlng.longitude + (lon * 1.24)
        : (userRequestData.isNotEmpty)
            ? (double.tryParse(userRequestData['pick_lng'].toString()) ?? 0.0) + (lon * 1.24)
            : 0.0;
    ...
    _cachedDriversQuery = FirebaseDatabase.instance
        .ref('drivers')
        .orderByChild('g')
        .startAt(lower)
        .endAt(higher);
    _cachedDriversStream = _cachedDriversQuery!.onValue.asBroadcastStream();
  }
                                                                        ...
                                                                        Column(
                                                                          ...
                                                                          children: [
                                                                            Text(
                                                                              ...
                                                                              style: GoogleFonts.poppins(fontSize: media.width * sixteen, color: textColor, fontWeight: FontWeight.w500),
                                                                            ),
                                                                                          ...
                                                                                          Column(
                                                                                            ...
                                                                                            children: [
                                                                                              SizedBox(
                                                                                                ...
                                                                                                child: Text(
                                                                                                  sosData[i]['name'],
                                                                                                  style: GoogleFonts.poppins(fontSize: media.width * fourteen, color: textColor, fontWeight: FontWeight.w500),
                                                                                                ),
                                                                                              ),
                                                                                              SizedBox(
  ...
  Widget _buildCancelOverlay(Size media) {
  ...
  Widget _buildLoadingOverlay(Size media) {
  ...
  // ── Floating Fare Meter Badge ────────────────────────────────────────────
  Widget _buildFareMeter(Size media) {
    final bool accepted = userRequestData.isNotEmpty &&
        userRequestData['accepted_at'] != null;
    final bool completed = userRequestData.isNotEmpty &&
        (userRequestData['is_completed'].toString() == '1' ||
            userRequestData['is_completed'] == 1);

    if (!accepted || completed) return const SizedBox.shrink();

    final bool tripStarted = userRequestData['is_trip_start'].toString() == '1' ||
    ...
    final String currencySymbol =
        userRequestData['requested_currency_symbol']?.toString() ?? '';
    final String fareAmount =
        userRequestData['running_fare']?.toString() ?? '0.00';

    String phaseLabel;
    Color phaseDotColor;
    if (tripStarted) {
      phaseLabel = 'Trip in progress';
      phaseDotColor = const Color(0xFF00C853); // green
    } else if (driverArrived) {
      phaseLabel = 'Driver waiting';
      phaseDotColor = const Color(0xFFFFAB00); // amber
    } else {
      phaseLabel = 'Driver en route';
      phaseDotColor = const Color(0xFF2196F3); // blue
    }

    return Positioned(
      bottom: media.width * 0.42,
      left: 0,
      right: 0,
      child: Center(
          ...
          child: Container(
            ...
            child: Row(
              ...
              children: [
                // Pulsing dot
                Container(
                  width: media.width * 0.022,
                  height: media.width * 0.022,
                  decoration: BoxDecoration(
                    color: phaseDotColor,
                    shape: BoxShape.circle,
                    boxShadow: [
                      BoxShadow(
                        color: phaseDotColor.withOpacity(0.6),
                        blurRadius: 6,
                        spreadRadius: 2,
                      ),
                    ],
                                                  ...
                                                  child: fm.FlutterMap(
                                                    ...
                                                    children: [
                                                      ...
                                                      fm.PolylineLayer(
                                                        polylines: [
                                                          fm.Polyline(
                                                              points: fmpoly.cast<fmlt.LatLng>(),
                                                              color:
                                                                  Colors.blue,
                                                              strokeWidth: 4),
                                                        ],
                                                                              ...
                                                                              Container(
                                                                                  ...
                                                                                  child: (userRequestData.isNotEmpty)
                                                                                      ? Text(
                                                                                          ...
                                                                                          softWrap: false,
                                                                                          style: GoogleFonts.poppins(color: buttonText, fontSize: (platform == TargetPlatform.android) ? media.width * twelve : media.width * sixteen),
                                                                                        )
  ...
  List<Widget> _buildBottomPanels(BuildContext context, Size media) {
    return [
                                                                                                ...
                                                                                                child: Column(
                                                                                                  ...
                                                                                                  children: [
                                                                                                    ...
                                                                                                    Text(
                                                                                                      ...
                                                                                                      style: GoogleFonts.poppins(fontSize: media.width * fourteen, fontWeight: FontWeight.w500, color: greyText),
                                                                                                    ),
                                                                                                  ],
                                                                                                ),
                                                                                              ),
                                                                                                                                            ...
                                                                                                                                            : Row(
                                                                                                                                                ...
                                                                                                                                                children: [
                                                                                                                                                  Text(
                                                                                                                                                    ...
                                                                                                                                                    style: GoogleFonts.poppins(fontSize: media.width * fourteen, color: (choosenVehicle != i) ? Colors.white : Colors.black, fontWeight: FontWeight.w500),
                                                                                                                                                  ),
                                                                                                                                                  Column(
                                                                                                                                                    children: [
                                                                                                                                                      Text(
                                                                                          ...
                                                                                          child: Row(
                                                                                            ...
                                                                                            children: [
                                                                                              SizedBox(
                                                                                                ...
                                                                                                child: Text(
                                                                                                  addressList[0].name,
                                                                                                  style: GoogleFonts.poppins(fontSize: media.width * twelve, color: buttonColor, fontWeight: FontWeight.w500),
                                                                                                  maxLines: 1,
                                                                                                  overflow: TextOverflow.ellipsis,
                                                                                                ),
                                                                                              ),
                                                                                        ...
                                                                                        (addressList[0].instructions != null)
                                                                                            ? SizedBox(
                                                                                                ...
                                                                                                child: Text(
                                                                                                  ...
                                                                                                  style: GoogleFonts.poppins(fontSize: media.width * twelve, color: verifyDeclined, fontWeight: FontWeight.w500),
                                                                                                  maxLines: 1,
                                                                                                  overflow: TextOverflow.ellipsis,
                                                                                                ))
                                                                                            : Container()
                                                                                        ...
                                                                                        children: [
                                                                                          SizedBox(
                                                                                            ...
                                                                                            child: Text(
                                                                                              ...
                                                                                              style: GoogleFonts.poppins(fontSize: media.width * twelve, color: buttonColor),
                                                                                              maxLines: 1,
                                                                                              overflow: TextOverflow.ellipsis,
                                                                                            ),
                                                                                          ),
                                                                                ...
                                                                                child: Row(
                                                                                  ...
                                                                                  children: [
                                                                                    (userDetails['show_ride_later_feature'] == true && ((widget.type == null) ? (etaDetails[choosenVehicle]['enable_bidding'] == null || etaDetails[choosenVehicle]['enable_bidding'] == false) : true) && isOutStation == false)
                                                                                        ? InkWell(
                                                                                            onTap: () async {
                                                                                               if (((rentalOption.isEmpty && (double.parse(etaDetails[choosenVehicle]['user_wallet_balance'].toString()) >= double.parse(etaDetails[choosenVehicle]['total'].toString()) && etaDetails[choosenVehicle]['has_discount'] == false) || (rentalOption.isEmpty && etaDetails[choosenVehicle]['has_discount'] == true && double.parse(etaDetails[choosenVehicle]['user_wallet_balance'].toString()) >= double.parse(etaDetails[choosenVehicle]['discounted_totel'].toString()))) || (rentalOption.isEmpty && etaDetails[choosenVehicle]['payment_type'].toString().split(',').toList()[payingVia] != 'wallet')) || ((rentalOption.isNotEmpty && (double.parse(etaDetails[0]['user_wallet_balance'].toString()) >= double.parse(rentalOption[choosenVehicle]['fare_amount'].toString())) && rentalOption[choosenVehicle]['has_discount'] == false) || (rentalOption.isNotEmpty && rentalOption[choosenVehicle]['has_discount'] == true && double.parse(etaDetails[0]['user_wallet_balance'].toString()) >= double.parse(rentalOption[choosenVehicle]['discounted_totel'].toString())) || rentalOption.isNotEmpty && rentalOption[choosenVehicle]['payment_type'].toString().split(',').toList()[payingVia] != 'wallet')) {
                                                                                    ...
                                                                                    Button(
                                                                                        ...
                                                                                        width: ((userDetails['show_ride_later_feature'] == true))
                                                                                            ...
                                                                                            : media.width * 0.89,
                                                                                        onTap: () async {
                                                                                          if ((widget.type == 2) || (((rentalOption.isEmpty && (double.parse(etaDetails[choosenVehicle]['user_wallet_balance'].toString()) >= double.parse(etaDetails[choosenVehicle]['total'].toString()) && etaDetails[choosenVehicle]['has_discount'] == false) || (rentalOption.isEmpty && etaDetails[choosenVehicle]['has_discount'] == true && double.parse(etaDetails[choosenVehicle]['user_wallet_balance'].toString()) >= double.parse(etaDetails[choosenVehicle]['discounted_totel'].toString()))) || (rentalOption.isEmpty && etaDetails[choosenVehicle]['payment_type'].toString().split(',').toList()[payingVia] != 'wallet')) || ((rentalOption.isNotEmpty && (double.parse(etaDetails[0]['user_wallet_balance'].toString()) >= double.parse(rentalOption[choosenVehicle]['fare_amount'].toString())) && rentalOption[choosenVehicle]['has_discount'] == false) || (rentalOption.isNotEmpty && rentalOption[choosenVehicle]['has_discount'] == true && double.parse(etaDetails[0]['user_wallet_balance'].toString()) >= double.parse(rentalOption[choosenVehicle]['discounted_totel'].toString())) || rentalOption.isNotEmpty && rentalOption[choosenVehicle]['payment_type'].toString().split(',').toList()[payingVia] != 'wallet'))) {
                                                                                          ...
                                                                                          Column(
                                                                                            ...
                                                                                            children: [
                                                                                              Text(
                                                                                                ...
                                                                                                style: GoogleFonts.poppins(fontSize: media.width * fourteen, fontWeight: FontWeight.w500),
                                                                                              ),
                                                                                              Text(
                                  ...
                                  (noDriverFound == false &&
                                          userRequestData.isNotEmpty &&
                                              (userRequestData['is_later'] == null ||
                                               userRequestData['is_later'].toString() == '0') &&
                                                                                                              ...
                                                                                                              child: Column(
                                                                                                                ...
                                                                                                                children: [
                                                                                                                  Text(
                                                                                                                    ...
                                                                                                                    style: GoogleFonts.poppins(fontSize: media.width * twelve, color: textColor, fontWeight: FontWeight.w500),
                                                                                                                    textAlign: TextAlign.center,
                                                                                                                    maxLines: 1,
                                                                                                                  ),
                                                                                                                  SizedBox(
                                                                              ...
                                                                              InkWell(
                                                                                ...
                                                                                child: Container(
                                                                                  ...
                                                                                  child: Text(
                                                                                    // '-10',
                                                                                    (userDetails['bidding_amount_increase_or_decrease'].toString().contains('.')) ? '-${double.parse(userDetails['bidding_amount_increase_or_decrease'].toString())}' : '-${int.parse(userDetails['bidding_amount_increase_or_decrease'].toString())}',
                                                                                    style: GoogleFonts.poppins(fontSize: media.width * fourteen, fontWeight: FontWeight.w500, color: Colors.white),
                                                                                  ),
                                                                                ),
                                                                              ),
                                                                              SizedBox(
                                              ...
                                              child: Container(
                                                ...
                                                child: Column(
                                                  children: [
                                                    ...
                                                    SizedBox(
                                                      height: media.width * 0.4,
                                                      child: Image.asset('assets/images/ridesearching.gif',
                                                        fit: BoxFit.contain,
                                                      ),
                                                    ),
                                                    SizedBox(
                                                    ...
                                                    Container(
                                                      ...
                                                      child: Container(
                                                        height:
                                                            media.width * 0.048,
                                                        width: (timing == null || maxWaitTime <= 0)
                                                            ? 0.0
                                                            : (media.width *
                                                                0.9 *
                                                                (timing! / maxWaitTime)),
                                                                                  ...
                                                                                  text: userRequestData['requested_currency_symbol'] + ' ' +
                                                                                      ...
                                                                                      : (userRequestData['is_bid_ride'] == 1)
                                                                                        ...
                                                                                        : (userRequestData['discounted_total'] != null)
                                                                                          ? userRequestData['discounted_total'].toString()
                                                                          ...
                                                                          Container(
                                                                            ...
                                                                            child: (addressList[i].address.isNotEmpty)
                                                                                ? Text(
                                                                                    ...
                                                                                    overflow: TextOverflow.fade,
                                                                                    softWrap: false,
                                                                                    style: GoogleFonts.poppins(fontSize: (platform == TargetPlatform.android) ? media.width * twelve : media.width * sixteen, color: buttonText),
                                                                                  )
                                                                                : Container(),
                                                                          ),
                                                                          const SizedBox(
  ...
  Widget build(BuildContext context) {
    ...
    // Start fare timer whenever ride is active
    if (userRequestData.isNotEmpty &&
        userRequestData['accepted_at'] != null &&
        _fareTimer == null) {
      _startFareTimer();
                ...
                child: ValueListenableBuilder(
                  ...
                  if (cancelRequestByUser == true) {
                    WidgetsBinding.instance.addPostFrameCallback((_) {
                       if (mounted && cancelRequestByUser == true) {
                            myMarker.clear();
                            polyline.clear();
                    ...
                    WidgetsBinding.instance.addPostFrameCallback((_) {
                       if (mounted && requestCancelledByDriver == true && polyGot == false) {
                            polyGot = true;
                            getPolylines('', '', '', '');
                       }
                    });
                            ...
                            builder:
                                      ...
                                      if (userRequestData != {}) {
                                        if (userRequestData['arrived_at'] ==
                                          ...
                                          if (polyline.isEmpty &&
                                            ...
                                            LatLngBounds bound;
                                            if (userRequestData.isNotEmpty) {
                                              double dLat = double.tryParse(driverData['l'][0].toString()) ?? 0.0;
                                              double dLng = double.tryParse(driverData['l'][1].toString()) ?? 0.0;
                                              double pLat = double.tryParse(userRequestData['pick_lat'].toString()) ?? 0.0;
                                              double pLng = double.tryParse(userRequestData['pick_lng'].toString()) ?? 0.0;
                                              if (dLat > pLat && dLng > pLng) {
                                                bound = LatLngBounds(
                                                    southwest: LatLng(pLat, pLng),
                                                    northeast: LatLng(dLat, dLng));
                                              } else if (dLng > pLng) {
                                                bound = LatLngBounds(
                                                    southwest: LatLng(dLat, pLng),
                                                    northeast: LatLng(pLat, dLng));
                                              } else if (dLat > pLat) {
                                                bound = LatLngBounds(
                                                    southwest: LatLng(pLat, dLng),
                                                    northeast: LatLng(dLat, pLng));
                                              } else {
                                                bound = LatLngBounds(
                                                    southwest: LatLng(dLat, dLng),
                                                    northeast: LatLng(pLat, pLng));
                                              }
                              ...
                              return Stack(
                                ...
                                children: [
                                  ...
                                  ..._buildBottomPanels(context, media),
                                  _buildFareMeter(media),
                                ],
                              );
                            });
                      });
  ...
  animateCar(
    ...
    if (animationControllers[markerid] != null) {
      animationControllers[markerid]!.dispose();
    }
    animationControllers[markerid] = AnimationController(
    ...
    _animation = tween.animate(animationControllers[markerid]!)
      ..addListener(() async {
        myMarker
    ...
    animationControllers[markerid]!.forward();
